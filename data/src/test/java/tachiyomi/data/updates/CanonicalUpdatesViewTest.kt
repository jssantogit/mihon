...
class CanonicalUpdatesViewTest {
    ...
    fun setUp() = runBlocking {
        ...
        nativeLibraryDirectory = Files.createTempDirectory("tsuzuki-updates-sqlite-jdbc")
        val nativeLibrary = nativeLibraryDirectory!!.resolve("libsqlitejdbc.so")
        CanonicalUpdatesViewTest::class.java
    ...
    @Test
    fun `canonical membership deduplicates source representations and chooses preferred operational variant`() =
        runBlocking<Unit> {
            val rows = database.updatesViewQueries.getRecentUpdatesWithFilters(
                after = 0L,
                limit = 500L,
                read = null,
                started = null,
                bookmarked = null,
                hideExcludedScanlators = 0L,
                includedEmpty = true,
                excludedEmpty = true,
    ...
    private suspend fun seedCanonicalUpdate() {
