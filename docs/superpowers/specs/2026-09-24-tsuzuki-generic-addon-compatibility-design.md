...
7. Reader-initiated Find/Add Reading Source is a temporary flow. Dismissing its binding sheet and pressing Back once returns to the same canonical chapter in the Reader, retaining the current reading position whenever Android preserves it. If Android recreates the Reader, restore the chapter and reading state rather than requiring the original Activity object. Search or binding alone must not change canonical progress or source preference. This supersedes the earlier plan to clear the Reader route; direct title-detail navigation from the library retains its normal Back behavior.
