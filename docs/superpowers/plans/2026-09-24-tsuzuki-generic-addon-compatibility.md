...
- [ ] **Approved navigation correction (2026-09-25):** the prior Reader-route clearing requirement above is superseded. Reader-initiated discovery is temporary: after dismissing the existing binding sheet, one Back resumes the same canonical chapter and reading position when preserved. Verify both cold and warm Activity stacks (`ReaderActivity` is `singleTask`, `MainActivity` is `singleTop`), avoid changing normal library-origin navigation, and do not mutate progress or preference merely by searching/binding. Prefer reuse of the binding sheet in the Reader over a second discovery/resolver implementation if this avoids destructive `CLEAR_TOP` behavior. Instrumented checks must distinguish original Reader resumption from correct restoration after system recreation, require a single sheet opening, and reject malformed identity. Validate with CI v2.1 and offline Android navigation workflow (`live_probe=false`); no MangaFire/MangaBall probe in this slice. At most two focused correction rounds.
