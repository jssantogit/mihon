...
- First full CI verified on `1cc1a5dd` (run `36204984805`): Format, Domain/App/Data/Core tests, SQLDelight, Supabase and Release Compile green. Incremental manual-link commit `c7fa582a` passed App/Domain tests but failed formatting on two style-only lines; successor fixes that and adds the explicit ready-state fallback; fresh CI is required before merge.
- Explicit Link Add-on now emits typed `BindingRefreshRequest` events with the exact persisted editions. Reader and title detail consume the scoped stream, refresh at most two editions in parallel and surface the first verified current-chapter option without waiting for other sibling inventories. The legacy title-wide `bindingChanges` stream and `refreshAfterBinding` method remain for compatibility, but are not used by these production UI collectors. Consecutive manual batches use `collect`, not `collectLatest`, so a later batch cannot cancel an earlier edition's refresh.
- The ready selector also offers “Find more reading sources” for a different language even when one readable option already exists. This is user-initiated and does not start a second automatic search.
