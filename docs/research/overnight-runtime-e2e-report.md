...
- Pending: push this bounded diagnostic and inspect one opt-in Android emulator run. It must remain offline/focused: no live MangaFire request. At report time no new emulator evidence exists; the exact operation producing the previous `SQLException` remains unknown until this run.
