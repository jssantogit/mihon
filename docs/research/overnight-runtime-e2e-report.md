...
- Starting from `5074b2efb1ce5902f056b3ee7274e342e46916ba` with remote equal to local, added five independently emitted phases to the offline instrumented persistence probe: `DRIVER_CREATE`, `DATABASE_CREATE`, `TITLE_INSERT`, `TITLE_READ`, and `DRIVER_CLOSE`. The database phase now explicitly executes a harmless `SELECT 1` through the production configured driver so lazy database opening/schema initialization failures cannot be attributed to a later title insert.
