...
- A narrowly approved correction now awaits the outer driver query and each cursor-advance result, and emits `unknown` schema states on query failure instead of incorrectly reporting every object missing. It changes no production code and makes no live requests. The corrected offline run is pending; until then, schema state and the original insert cause remain unknown.
